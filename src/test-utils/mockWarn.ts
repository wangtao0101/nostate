  ...
  expect.extend({
    toHaveBeenWarned(received: string) {
      ...
      if (passed) {
        return {
          pass: true,
          message: (): string => `expected "${received}" not to have been warned.`,
        };
      } else {
    ...
    toHaveBeenWarnedLast(received: string) {
      ...
      if (passed) {
        return {
          pass: true,
          message: (): string => `expected "${received}" not to have been warned last.`,
        };
      } else {
        const msgs = warn.mock.calls.map(args => args[0]).join('\n - ');
        return {
    ...
    toHaveBeenWarnedTimes(received: string, n: number) {
      ...
      if (found === n) {
        return {
          pass: true,
          message: (): string => `expected "${received}" to have been warned ${n} times.`,
        };
      } else {
        return {
          pass: false,
